...
def main(argv=None):
    ...
    bins = range(0, 25, 2)
    axs[1].hist(n_tokens, bins=range(0,25), zorder=3, edgecolor="black")
    axs[1].axvline(x=np.mean(n_tokens) + 0.5, color="r", label="mean", zorder=3)
    axs[1].set_xticks(ticks=np.array(bins) + 0.5, labels=bins)
    ...
    bins = range(1, 13)
    axs[2].hist(n_phonemes_word, bins=bins, zorder=3, edgecolor="black", density=True)
    axs[2].axvline(x=np.mean(n_phonemes_word) + 0.5, color="r", label="mean", zorder=3)
    axs[2].set_xticks(ticks=np.array(bins) + 0.5, labels=bins)
    axs[2].grid(True, axis="y", zorder=0)
    axs[2].legend()
    axs[2].set_ylabel("Frequencies")
    axs[2].set_xlabel("Number of phonemes per word")
    axs[2].set_title("Number of phonemes per word")

    bins = range(0, 50, 4)
    axs[4].hist(n_phonemes_line, bins=range(0,50), zorder=3, edgecolor="black", density=True)
    axs[4].axvline(x=np.mean(n_phonemes_line) + 0.5, color="r", label="mean", zorder=3)
    axs[4].set_xticks(ticks=np.array(bins) + 0.5, labels=bins)
    axs[4].grid(True, axis="y", zorder=0)
    axs[4].legend()
    axs[4].set_ylabel("Frequencies")
    axs[4].set_xlabel("Number of phonemes per line")
    axs[4].set_title("Number of phonemes per line")

    bins = range(1,8)
    axs[3].hist(n_stress_word, bins=bins, zorder=3, edgecolor="black", density=True)
    axs[3].axvline(x=np.mean(n_stress_word) + 0.5, color="r", label="mean", zorder=3)
    axs[3].set_xticks(ticks=np.array(bins) + 0.5, labels=bins)
    axs[3].grid(True, axis="y", zorder=0)
    axs[3].legend()
    axs[3].set_ylabel("Frequencies")
    axs[3].set_xlabel("Number of syllables per word")
    axs[3].set_title("Number of syllables per word")

    bins = range(0, 22, 2)
    axs[5].hist(n_stress_line, bins=range(0,22), zorder=3, edgecolor="black", density=True)
    axs[5].axvline(x=np.mean(n_stress_line) + 0.5, color="r", label="mean", zorder=3)
    axs[5].set_xticks(ticks=np.array(bins) + 0.5, labels=bins)
    axs[5].grid(True, axis="y", zorder=0)
    axs[5].legend()
    axs[5].set_ylabel("Frequencies")
    axs[5].set_xlabel("Number of syllables per line")
